...
vec3 MaterialRoughReflectionDirection(
	...
	return normalize(reflect(incidentView, normalView));
}
...
MaterialReflectionTrace TraceMaterialReflection(const in vec3 viewPos, const in vec3 reflectedDir, const in int traceSteps) {
	...
	for (int i = 0; i < FOXY_MATERIAL_REFLECTION_STEPS; i++) {
		if (i >= traceSteps) break;
		float t = (float(i) + jitter) / float(traceSteps);
		...
		if (sampleValid > 0.5) {
			...
			if (delta >= -thickness * 0.25 && delta <= thickness) {
				...
				if (previousValid > 0.5 && previousDelta < 0.0 && delta >= 0.0 && abs(crossingDenominator) > 1.0e-5) {
					refinedT = mix(previousValidT, t, clamp(-previousDelta / crossingDenominator, 0.0, 1.0));
				}
...
vec4 TraceMaterialReflectionSignal(
	...
	float traceSelected = 1.0;
	#if FOXY_MATERIAL_REFLECTION_HIGH_QUALITY == 0
		ivec2 tracePixel = ivec2(gl_FragCoord.xy);
		int pixelPhase = (tracePixel.x & 1) | ((tracePixel.y & 1) << 1);
		traceSelected = float(pixelPhase == (frameCounter & 3));
	#endif
	if (traceSelected < 0.5) return vec4(0.0);
	...
	#endif
	return vec4(max(reflectedColor, vec3(0.0)), 1.0);
}
